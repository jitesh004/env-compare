...
def is_environment_specific(key, env1, env2):
    ...
    # Check if the key contains environment-specific indicators
...
def extract_diff(diff, value1, value2, key_path):
    def format_key(path):
        return path.replace("root.", "").replace("root[", "[").replace("]", "]").replace("'", "")
    ...
    def handle_nested_diffs(old_value, new_value, base_key=""):
        ...
        if isinstance(old_value, dict) and isinstance(new_value, dict):
            ...
            if isinstance(old_value, str) and isinstance(new_value, str):
                ...
                differences = [f"{o} => {n}" for o, n in zip(old_parts, new_parts) if o != n]
        ...
        for key in all_keys:
            ...
            if value1 != "undefined" and value2 != "undefined":
                ...
                if diff:
                    ...
                    status = "Values differ as expected for environments" if row_class == "blue" else "Unexpected difference, please review"
...
def write_comparison_to_html(comparison_results, summary, template_path, output_file, env1, env2, branch_name, commit_id, commit_message, file1_path, file2_path):
    try:
        ...
        <p><strong>Comparing Files: {file1_path} & {file2_path}</strong></p>
        ...
        for key, value1, value2, exact_diff, row_class, status in comparison_results:
            ...
            <tr class="{row_class}">
                ...
                <td><pre style="word-break: break-all;">{exact_diff}</pre></td>
